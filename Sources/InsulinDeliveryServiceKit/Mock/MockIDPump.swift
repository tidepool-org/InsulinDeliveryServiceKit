...
open class MockIDPump: IDPumpComms, @unchecked Sendable {
    ...
    private let lockedStatus: Locked<MockIDPumpStatus>
    ...
    public var status: MockIDPumpStatus {
        get {
    ...
    public var willDisconnectAfterAuthentication: Bool { true }
    ...
    public func unpairPumpBase(completion: @escaping ProcedureResultCompletion) {
        loggingDelegate?.logConnectionEvent(#function)

        checkCommsStateAndRespond(response: completion) { [weak self] in
            self?.reset()
        }
    }
    ...
    public func initiateBolus(_ amount: Double) {
        ...
        loggingDelegate?.logReceiveEvent("Quick Bolus Initiated")
...
extension MockIDPump {
    private func triggerExpirationIfNeeded(at now: Date = Date()) {
    ...
    private func triggerStoppedAnnunciationIfNeeded(at now: Date = Date()) {
